#!/usr/bin/env python
"""
Final Python LSP WebSocket Server

This script creates a WebSocket server that connects to the Python Language Server
and properly handles document URIs for multi-file support.
"""
...
logger = logging.getLogger('final_lsp')
...
async def handle_client(websocket):
    ...
    if not lsp_process:
        logger.error(f"Failed to start LSP process for client {client_info}")
        await websocket.send(json.dumps({
            "jsonrpc": "2.0",
            "method": "window/showMessage",
            "params": {
                "type": 1,  # Error
                "message": "Failed to start Python Language Server"
            }
        }))
        return
    ...
    try:
        ...
        await websocket.send(json.dumps({
            ...
            "params": {
                "type": 3,  # Info
                "message": "Connected to Final Python LSP Server"
            }
        }))
        ...
        async for message_str in websocket:
            try:
                ...
                # Special handling for document URIs to support multi-file
                if 'params' in message and 'textDocument' in message.get('params', {}):
                    # Ensure proper URI handling for multi-file support
                    if 'uri' in message['params']['textDocument']:
                        uri = message['params']['textDocument']['uri']
                        # Parse the URI to extract the filename
                        parsed_uri = urlparse(uri)
                        filename = os.path.basename(parsed_uri.path)
                        # Keep the URI format but ensure it's properly structured
                        if not uri.startswith('file:///'):
                            # Convert to a proper file URI
                            message['params']['textDocument']['uri'] = f"file:///workspace/{filename}"
...
async def main():
    ...
    # Start the WebSocket server with the correct handler signature
